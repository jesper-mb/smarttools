	...
	const categoryData: Array<{ key: CategoryKey; icon: string; name: string; description: string }> = [
		{
			...
			icon: '⏱',
			name: 'Tiempo',
			description: 'Cronómetro, temporizadores, cuenta regresiva'
		},
		{
			key: 'text',
			icon: 'Aa',
			name: 'Texto',
			description: 'Contar palabras, ordenar, limpiar'
		},
		{
			key: 'calc',
			icon: '%',
			name: 'Calculadoras',
			description: 'Porcentaje, edad, unidades'
		},
		{
			key: 'generators',
			icon: '✦',
			name: 'Generadores',
			description: 'Aleatorio, contraseñas, códigos QR'
		},
		{
			key: 'dev',
			icon: '{ }',
			name: 'Desarrollador',
			description: 'Base64, JSON, hashing, codificación'
		},
		{
			key: 'social',
			icon: '@',
		...
		},
		{
			...
			icon: '→',
			name: 'Viaje',
			description: 'Equipaje, planificación de jetlag'
		}
	];
				...
				<div class="py-8 lg:py-16 lg:pl-16 lg:border-l-2 border-neutral-200">
					<div class="animate-fade-in-up max-w-md" style="animation-delay: 100ms; opacity: 0;">
						...
						<p class="text-xs text-neutral-400 mt-3 font-sans">
							Pulsa <kbd class="px-1.5 py-0.5 bg-neutral-100 border border-neutral-300 text-xs font-mono">K</kbd>
							<span class="ml-1">para buscar desde cualquier lugar</span>
						</p>
					</div>
				</div>
			</div>
	...
	<div class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onclick={() => showSuggestTool = false}>
		<div class="bg-white border-2 border-neutral-950 max-w-md w-full p-6" onclick={(e) => e.stopPropagation()}>
			<div class="flex items-center justify-between mb-6">
				<h2 class="text-xl font-bold">Sugerir una herramienta</h2>
				<button onclick={() => showSuggestTool = false} class="text-2xl text-neutral-400 hover:text-neutral-950">&times;</button>
			</div>

			<p class="text-sm text-neutral-500 mb-6">
				¿Falta una herramienta? ¡Dinos y la construiremos en 10 minutos!
			</p>

			<div class="space-y-4">
				<div>
					<label class="block text-xs font-bold uppercase tracking-wider text-neutral-400 mb-2">Nombre de la herramienta</label>
					<input
						type="text"
						bind:value={suggestedToolName}
						placeholder="ej. Vista previa de Markdown"
						class="w-full px-4 py-3 border-2 border-neutral-200 focus:border-orange-500 outline-none"
					/>
				</div>
				<div>
					<label class="block text-xs font-bold uppercase tracking-wider text-neutral-400 mb-2">Descripción</label>
					<textarea
						bind:value={suggestedToolDesc}
						placeholder="¿Qué debería hacer la herramienta?"
			...
			<div class="mt-6 flex gap-3">
				<button
					onclick={() => {
						alert(`¡Gracias! Empezaremos a trabajar en: ${suggestedToolName}`);
			...
			<p class="mt-4 text-xs text-neutral-400 text-center">
				Construimos nuevas herramientas lo más rápido posible. Tiempo promedio: 10 minutos.
			</p>
		</div>
	</div>
{/if}
