	...
	const categoryData: Array<{ key: CategoryKey; icon: string; name: string; description: string }> = [
		{
			...
			icon: '⏱',
			name: 'Time',
			description: 'Stopwatch, timers, countdowns'
		},
		{
			key: 'text',
			icon: 'Aa',
			name: 'Text',
			description: 'Word count, sort, clean text'
		},
		{
			key: 'calc',
			icon: '%',
			name: 'Calculators',
			description: 'Percentage, age, unit conversion'
		},
		{
			key: 'generators',
			icon: '✦',
			name: 'Generators',
			description: 'Random, passwords, QR codes'
		},
		{
			key: 'dev',
			icon: '{ }',
		...
		},
		{
			...
			icon: '→',
			name: 'Travel',
			description: 'Luggage, jetlag planning'
		}
	];
				...
				<div class="py-12 lg:py-16 lg:pr-16">
					<div class="animate-fade-in-up">
						...
						<p class="text-lg sm:text-xl text-neutral-600 font-sans max-w-sm mb-8 leading-relaxed">
							Simple tools that just work.<br/>
							<span class="text-neutral-400">No registration. No ads.</span>
						</p>
	...
	<div class="py-16 bg-neutral-50 border-b border-neutral-200">
		<div class="max-w-6xl mx-auto px-6 sm:px-8 lg:px-12">
			<div class="flex items-baseline justify-between mb-8">
				<h2 class="text-xs font-bold uppercase tracking-widest text-neutral-400">Categories</h2>
				<span class="text-xs text-neutral-300 font-mono">4 categories</span>
			</div>
		...
		<div class="max-w-6xl mx-auto px-6 sm:px-8 lg:px-12">
			<div class="grid lg:grid-cols-3 gap-8 lg:gap-12">
				...
				<div>
					...
					<p class="text-sm text-neutral-400 font-sans leading-relaxed">
						Every tool has shortcuts. Space to start, R to reset.
					</p>
				</div>
				<div>
					<div class="text-orange-500 font-mono text-sm mb-3">03</div>
					<h3 class="text-lg font-bold mb-2">No clutter</h3>
					<p class="text-sm text-neutral-400 font-sans leading-relaxed">
						No pop-ups, no cookies, no ads. Just the tool.
					</p>
				</div>
			</div>
		</div>
		...
		<div class="max-w-6xl mx-auto px-6 sm:px-8 lg:px-12">
			<div class="flex flex-wrap justify-center gap-12 sm:gap-16 lg:gap-24">
				...
				<div class="text-center">
					...
					<div class="text-xs text-neutral-400 font-sans mt-1 uppercase tracking-wider">Ads</div>
				</div>
				<div class="text-center">
					<div class="text-3xl sm:text-4xl font-bold text-neutral-950 font-mono">2</div>
					<div class="text-xs text-neutral-400 font-sans mt-1 uppercase tracking-wider">Languages</div>
				</div>
				<div class="text-center">
					<div class="text-3xl sm:text-4xl font-bold text-orange-600 font-mono">∞</div>
					<div class="text-xs text-neutral-400 font-sans mt-1 uppercase tracking-wider">Free</div>
				</div>
			</div>
		</div>
	</div>
	...
	<div class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onclick={() => showSuggestTool = false}>
		<div class="bg-white border-2 border-neutral-950 max-w-md w-full p-6" onclick={(e) => e.stopPropagation()}>
			...
			<div class="space-y-4">
				...
				<div>
					<label class="block text-xs font-bold uppercase tracking-wider text-neutral-400 mb-2">Description</label>
					<textarea
			...
			<div class="mt-6 flex gap-3">
				<button
					onclick={() => {
						...
						alert(`Thanks! We'll start working on: ${suggestedToolName}`);
			...
			<p class="mt-4 text-xs text-neutral-400 text-center">
				We build new tools as fast as possible. Average time: 10 minutes.
			</p>
		</div>
	</div>
{/if}
